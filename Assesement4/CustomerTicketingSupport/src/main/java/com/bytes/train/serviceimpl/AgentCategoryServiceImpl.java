...
public class AgentCategoryServiceImpl implements AgentCategoryService {
	...
	public List<Ticket> getSearch(int agentId, SearchCriteria searchCriteria) throws Exception {
		...
		if (("Open").equalsIgnoreCase(searchCriteria.getStatus())||("Closed").equalsIgnoreCase(searchCriteria.getStatus())) {
			...
			if(searchCriteria.getSubject().trim().length()!=0 && searchCriteria.getTicketId()>0) {
				System.out.println("Here Searching");
				 return ticketRepository.findByCategoryIdInAndStatusAndSubjectIgnoreCaseContainingOrTicketIdAndStatus(category,searchStatus,serachTicketSubject,serachTicketId,searchStatus);
			}
