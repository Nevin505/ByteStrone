...
// To get the list of Tickets Which belongs to Agents Categories
	...
	public ResponseEntity<Response> getParticularAgentTickets(@PathVariable String agentId) throws Exception {
		try {
			String encodedId = agentId; 
			...
			List<Ticket> agentsTickets = agentService.getParticularCategoryList(originalId);
...
//	Filters Open and Closed in a single Function
	...
	public ResponseEntity<Response> getfilteredtickets(@PathVariable("agentId") String agentId,
			@RequestBody FilterCreteria filterCreteria) {
		try {
//			String encodedId = agentId;
			byte[] decodedBytes = Base64.getDecoder().decode(agentId);
			String decodedId = new String(decodedBytes);
			int originalId = Integer.parseInt(decodedId);
			
			StatusEnum statusTicket = StatusEnum.valueOf(filterCreteria.getStatus());			
			List<Ticket> filterTickets = agentService.getfilteredTickets(originalId, statusTicket);
...
//	Search Tickets Based on  Number and Subject
	...
	public ResponseEntity<Response> searchTickets(@PathVariable("agentId") String agentId,
			@RequestBody SearchCriteria searchCriteria) {
		try {
			byte[] decodedBytes = Base64.getDecoder().decode(agentId);
			String decodedId = new String(decodedBytes);
			int originalId = Integer.parseInt(decodedId);
			List<Ticket> searchTickets = agentService.getSearch(originalId, searchCriteria);
	...
	public ResponseEntity<Response> agents(@PathVariable String agentId) {
		try {
			byte[] decodedBytes = Base64.getDecoder().decode(agentId);
			String decodedId = new String(decodedBytes);
			int originalId = Integer.parseInt(decodedId);
			
			List<Agent> agentList = agentService.getAgents(originalId);
...
//	To Close a Ticket which has been handled by a particular Agent
	...
	public ResponseEntity<Response> closeAsignedTickets(@PathVariable int ticketid, @PathVariable String agentId) {
		...
		try {
			...
			Ticket ticket = agentService.closeTickets(ticketid, originalId);
			if (ticket == null) {
...
//	To View Assigned Tickets by Agents
	...
	public ResponseEntity<Response> ticketAssigned(@PathVariable String agentId) {
		try {
			...
			List<Ticket> agentTicket = agentService.getAssignedTickets(originalId);
